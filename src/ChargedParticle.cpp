...
(const Eigen::VectorXd& x, KFBase::VERTEX_COMPONENT component) const {
  ...
  switch (component) {
  case KFBase::VERTEX_X:
    result(bi) = -sinP / qBc + sinA / qBc -
      pt2 * (1 + eta2) * ct * cosA / denom3;
    result(bi + 1) =
      -eta * pt2 * x(bi) * ct * cosA / denom3;
    result(bi + 2) =
      -(x(bi + 3) + r) * std::cos(x(bi + 2)) + r * cosA;
    result(bi + 3) = -sinP;
    result(_timeParam->getBeginIndex()) = r * w * cosA;
    break;
  case KFBase::VERTEX_Y:
    result(bi) = cosP / qBc - cosA / qBc -
      pt2 * (1 + eta2) * ct * sinA / denom3;
    result(bi + 1) =
      -eta * pt2 * x(bi) * ct * sinA / denom3;
    result(bi + 2) =
      -(x(bi + 3) + r) * std::sin(x(bi + 2)) + r * sinA;
    result(bi + 3) = cosP;
    result(_timeParam->getBeginIndex()) = r * w * sinA;
    break;
  case KFBase::VERTEX_Z:
    result(bi) = eta * ct / denominator -
      pt2 * eta * (1 + eta2) * ct / denom3;
    result(bi + 1) = x(bi) * ct / denominator -
      eta2 * pt2 * x(bi) * ct / denom3;
    result(bi + 4) = 1;
...
(const Eigen::VectorXd& x, KFBase::VERTEX_COMPONENT component) const {
  ...
  case KFBase::VERTEX_X:
    ...
    result(bi + 1, bi + 1) =
      -pt3 * ct * cosA / denom3 + 3. * pt5 * eta2 * ct * cosA / denom5 -
    ...
    result(_timeParam->getBeginIndex(), _timeParam->getBeginIndex()) = -r * w * w * sinA;
    break;
  case KFBase::VERTEX_Y:
    ...
    result(bi, bi + 1) = -3. * pt2 * eta * ct * sinA / denom3 +
      ...
      pt4 * eta * (1 + eta2) * ct2 * qBc *
      cosA / denom6;
    result(bi + 1, bi) = result(bi, bi + 1);
    result(bi, bi + 2) = -sinP / qBc + sinA / qBc -
      pt2 * a1 * ct * cosA / denom3;
    result(bi + 2, bi) = result(bi, bi + 2);
    result(bi, _timeParam->getBeginIndex()) =
      sinA * w / qBc -
      pt2 * a1 * sinA / denom3 -
    ...
    result(_timeParam->getBeginIndex(), bi + 2) = result(bi + 2, _timeParam->getBeginIndex());
    result(_timeParam->getBeginIndex(), _timeParam->getBeginIndex()) = r * w * w * cosA;
    break;
  case KFBase::VERTEX_Z:
    result(bi, bi) = -3. * x(bi) * eta * a1 * ct / denom3 +
      3. * pt3 * a2 * eta * ct / denom5;
    result(bi, bi + 1) = ct / denominator - pt2 * (1 + 4 * eta2) * ct / denom3 +
      3. * eta2 * a1 * pt4 * ct / denom5;
    result(bi + 1, bi) = result(bi, bi + 1);
    result(bi, _timeParam->getBeginIndex()) = eta / denominator - pt2 * eta * a1 / denom3;
    result(_timeParam->getBeginIndex(), bi) = result(bi, _timeParam->getBeginIndex());
    result(bi + 1, bi + 1) = -3. * eta * pt3 * ct / denom3 + 3. * eta * eta2 * pt5 * ct / denom5;
    result(bi + 1, _timeParam->getBeginIndex()) = x(bi) / denominator - eta2 * pt3 / denom3;
    result( _timeParam->getBeginIndex(), bi + 1) = result(bi + 1, _timeParam->getBeginIndex());
    break;
  }
  return result;
...
void KFCmd::ChargedParticle::setTimeParameter
